...
def checkout(request, lbID):
    ...
    if(book_instance.count > 0):
        ...
        checkedout = Checks_Out.objects.new_checkout(user=user, book=book_instance)
...
def returns(request, chID):
    ...
    if(bk):
        ...
        return render(request, "returns.html", {"result": True, "book": book, "user_id": request.session['user_id'], "user_name": request.session['user_name'],})
    else:
        return render(request, "returns.html", {"result": False, "book": book, "user_id": request.session['user_id'], "user_name": request.session['user_name'],})
